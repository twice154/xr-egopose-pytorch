...
def main():
    ...
    for ep in range(config.train_setting.epoch):
        ...
        for it, (img, p2d, p3d, action, heatmap) in tqdm(enumerate(test_data_loader), total=len(test_data_loader)):
            ...
            # AverageMeter Update
            errorAverageMeter.update(heatmap_prediction_loss.data.cpu().numpy())
        ...
        res = {'HeatmapPredictionMSE': errorAverageMeter.avg}
        ...
        copyfile("data/config.yml", os.path.join(os.getcwd(), config.eval.experiment_folder, str("epoch_" + str(ep)), "train_backbone_config.yml"))
        ...
        # torch.save(encoder, os.path.join(os.getcwd(), config.eval.experiment_folder, str("epoch_" + ep), config.eval.encoder_weight_file))
        # torch.save(decoder, os.path.join(os.getcwd(), config.eval.experiment_folder, str("epoch_" + ep), config.eval.decoder_weight_file))
        # torch.save(reconstructer, os.path.join(os.getcwd(), config.eval.experiment_folder, str("epoch_" + ep), config.eval.reconstructer_weight_file))
